...
/* 从缓冲区中取出第1个字符的ANSI编码值(英文字母用ASCII,汉字用GBK) */
static int AsciiGetCodeFrmBuf(unsigned char *pucBufStart, unsigned char *pucBufEnd, unsigned int *pdwCode)
{
...
/* 根据字符的ANSI编码值,有freetype/ascii/gbk这三种方法获得它的字符位图，AsciiEncodingInit先给g_tAsciiEncodingOpr添加freetype/ascii/gbk这三种PT_FontOpr结构体;*/
