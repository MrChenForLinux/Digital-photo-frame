...
/* 为"浏览页面"生成"目录或文件"区域中的图标和文字,就是显示目录内容 */
    ...
    for (i = 0; i < g_iDirFileNumPerCol; i++)
    {
        for (j = 0; j < g_iDirFileNumPerRow; j++)
        {
            if (iDirContentIndex < iDirContentsNumber)
            {
                ...
                k++;
                /* 显示目录或文件的名字 */          
                ret = MergerStringToCenterOfRectangleInVideoMem(atLayout[k].iTopLeftX, atLayout[k].iTopLeftY, atLayout[k].iBotRightX, atLayout[k].iBotRightY, (unsigned char *)aptDirContents[iDirContentIndex]->strName, ptVideoMem);               
                k++;
...
static int GenerateDirAndFileIcons(PT_PageLayout ptPageLayout)
{
    ...
    /* 从BMP文件里提取图像数据 */
    /* 1. 提取"fold_closed图标" */
    ret = GetPixelDatasForIcon(g_strDirClosedIconName, &tOriginIconPixelDatas);
    if (ret)
    {
    ...
    /* 2. 提取"fold_opened图标" */
    ret = GetPixelDatasForIcon(g_strDirOpenedIconName, &tOriginIconPixelDatas);
    if (ret)
    {
    ...
    /* 3. 提取"file图标" */
    ret = GetPixelDatasForIcon(g_strFileIconName, &tOriginIconPixelDatas);
    if (ret)
    {
        printf("GetPixelDatasForIcon %s error!\n", g_strFileIconName);
        return -1;
...
static void ShowBrowsePage(PT_PageLayout ptPageLayout)
{
	...
	ret = GeneratePage(ptPageLayout, ptVideoMem);
    ret = GenerateBrowsePageDirAndFile(g_iStartIndex, g_iDirContentsNumber, g_aptDirContents, ptVideoMem);
...
static void BrowsePageRun(PT_PageParams ptParentPageParams)
{
    ...
    int bUsedToSelectDir = 0;		/* 等于1说明进入的是setting界面 */
    ...
    /* 0. 获得要显示的目录的内容 */
    ret = GetDirContents(g_strCurDir, &g_aptDirContents, &g_iDirContentsNumber);
    if (ret)
    {
    					...
    					switch (iIndex)
    					{
    						case 0: /* "向上"按钮 */
    						{
                                ...
                                *pcTmp = '\0';
                                ...
                                g_iStartIndex = 0;
                                ret = GenerateBrowsePageDirAndFile(g_iStartIndex, g_iDirContentsNumber, g_aptDirContents, ptDevVideoMem);
