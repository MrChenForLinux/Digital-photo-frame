...
static int GenerateIntervalPageSpecialIcon(int dwNumber, PT_VideoMem ptVideoMem)
{
	...
	ret = MergerStringToCenterOfRectangleInVideoMem(g_tIntervalNumberLayout.iTopLeftX, g_tIntervalNumberLayout.iTopLeftY, g_tIntervalNumberLayout.iBotRightX, g_tIntervalNumberLayout.iBotRightY, (unsigned char *)strNumber, ptVideoMem);
...
static void ShowIntervalPage(PT_PageLayout ptPageLayout)
{
	...
	if (ptVideoMem == NULL)
	{
		printf("can't get video mem for interval page!\n");
		return;
	}
	...
	ret = GenerateIntervalPageSpecialIcon(g_iIntervalSecond, ptVideoMem);
	if (ret)
	{
		printf("GenerateIntervalPageSpecialIcon error!\n");
	}
